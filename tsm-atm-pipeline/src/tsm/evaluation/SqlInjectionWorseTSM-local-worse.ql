import javascript
import tsm.evaluation.SqlInjectionWorseTSMCustomizations
import tsm.sql.SqlInjectionTSMConfig
...
private import tsm.sql.SqlInjectionWorseRepresentations
private import tsm.NodeRepresentation

module SqlInjectionWorse = SqlInjection;
...
predicate sameLocationInfo(DataFlow::PathNode n1, DataFlow::PathNode n2) {
  ...
  query predicate compareVWvsWorseBoosted(int new, int missing, int same) {
    new =
      count(DataFlow::PathNode source, DataFlow::PathNode sink |
        exists(TSMConfig::Configuration cfg |
          ...
          not exists(
            SqlInjectionWorse::Configuration cfgV0, DataFlow::PathNode source2, DataFlow::PathNode sink2
          |
    ...
    missing =
      count(DataFlow::PathNode source, DataFlow::PathNode sink |
        exists(SqlInjectionWorse::Configuration cfgV0 |
  ...
  query predicate compareMissedVWvsWorseBoostedSinks(
    ...
    exists(SqlInjectionWorse::Configuration cfgV0, DataFlow::PathNode source2, DataFlow::PathNode sink2 |
      cfgV0.hasFlowPath(source2, sink2) and
  ...
  query predicate compareNewVWvsWorseBoostedSinks(
    ...
    exists(DataFlow::PathNode source, DataFlow::PathNode sink |
      exists(TSMConfig::Configuration cfg |
        cfg.hasFlowPath(source, sink) and
        not exists(
            SqlInjectionWorse::Configuration cfgVW, DataFlow::PathNode source2, DataFlow::PathNode sink2
        |
        ...
        rep = candidateRep(sinkNew.getNode(), _, true) and
        not exists( SqlInjectionWorse::Sink knownSink |
