...
DB_LOGGER = "orchestrator.dblogger"
...
class OrchestrationStep:
    ...
    def clean_error_callback(self, function, path, exec_info):
        self.logger.error(
            f"Error cleaning folder: {path}, exec_info: {exec_info}")
    ...
    def get_new_working_directories(self, query_name, working_dir):
        ...
        constraints_dir = os.path.join(
            working_dir, "constraints", name, optimizer_run_name)
        models_dir = os.path.join(
            working_dir, "models", name, optimizer_run_name)
        logs_dir = os.path.join(working_dir, "logs",
                                project_name, optimizer_run_name)
    ...
    def get_existing_working_directories(self, query_name, working_dir):
        ...
        patternToSearch = os.path.join(
            working_dir, "constraints", name) + "/{0}-*".format(query_name)
        ...
        if len(candidates) > 0:
            ...
            self.logger.info(
                f"Project dir: {project_name}/{optimizer_run_name}")
        else:
            raise ValueError(
                'Cannot find results directory for ' + patternToSearch)
