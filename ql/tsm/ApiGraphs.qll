...
module API {
  ...
  private module Impl {
    ...
    predicate rhs(TApiNode base, string lbl, DataFlow::Node rhs) {
      ...
      (
        ...
        |
          ...
          exists(Import imp, Module m, string prop, ASTNode ast|
            pred = imp.getImportedModuleNode() and
            m = imp.getImportedModule() and
            lbl = Label::member(prop) and
            m.exports(prop, ast) and
            rhs.getAstNode() = ast
          )
          or
    ...
    private CanonicalName getChild(CanonicalName cn,  string name) {
      result = cn.getAChild() and
      result.getName() = name
    }
