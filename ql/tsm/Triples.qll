...
module PropagationGraph {
  ...
  predicate isKnownSource(DataFlow::Node source) {
    exists(string url |
      url = any(KnownURL k).getURL("sources") and
      url = buildURL(source)
    )
  }
  ...
  predicate isKnownSink(DataFlow::Node sink) {
    exists(string url |
      url = any(KnownURL k).getURL("sinks") and
      url = buildURL(sink)
    )  }
  ...
  predicate isKnownSanitizer(DataFlow::Node sanitizer) {
    exists(string url |
      url = any(KnownURL k).getURL("sanitizers") and
  ...
  /**
   * A set of known sources, sinks, or sanitizers defined by its URL.
   *
   * Subclasses of this class are generated by the `code/compute_metrics.py` script.
   */
  abstract class KnownURL extends string {
    bindingset[this]
    KnownURL() { any() }

    /**
     * Gets a URL of a source, sink, or sanitizer in this set, depending on `t`:
     * `sources` gets a source URL, `sanitizers` a sanitizer URL, and
     * `sinks` a sink URL.
     */
    abstract string getURL(string t);
  }
